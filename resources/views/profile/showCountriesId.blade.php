<x-app-layout>
    ...
    <div class="main-page-countries">
        ...
        @if (
            $listGamesGroup->isEmpty() and
                $listGamesCup->isEmpty() and
                $listGamesFriendry->isEmpty() and
                $listOldGroup->isEmpty())
            <div class="container py-10 px-10 mx-0 min-w-full flex flex-col items-center">
                <button command="show-modal" commandfor="dialog"
                    class="!bg-red-500   text-white text-bold  py-2 px-4 ">Usuń Państwo</button>
            </div>
                            ...
                            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                <div class="sm:flex sm:items-start">
                                    ...
                                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                        <h3 id="dialog-title" class="text-base font-semibold text-gray-900">Usunięcie
                                            Państwa</h3>
                                        <div class="mt-2">
                                            <p class="text-sm text-gray-500">Czy na pewno chcesz usunąć tę Państwo?
                                                Wszystkie dane zostaną trwale usunięte. tej akcji nie można cofnąć.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        @if ($listGamesGroup->isEmpty() and $listGamesCup->isEmpty())
            <div class="container py-10 px-1  min-w-full ">
                ...
                <form action="{{ route('profile.changeGroup') }}" method="POST">
                    @csrf
                    @method('PUT')
                    <select name="groupChange" id="groupChange"
                        ...
                        <option value="0"
                            {{ empty(\App\Models\Group_forwarding::checkForwarding($idCountry)) ? 'selected' : '' }}>Nie
                            przepisuj do żadnej grupy</option>
                        @foreach (\App\Models\Group::showGroup() as $item)
                            @if (
                                !empty(\App\Models\Group_forwarding::checkForwarding($idCountry)) and
                                    \App\Models\Group_forwarding::checkForwarding($idCountry)->group_id == $item->id)
        ...
        @foreach ($listGamesGroup as $game)
            <div class=" px-4 py-2 ">
                @if (Auth::user()->status < 0 and session()->get('oldGroup') == 0)
                    ...
                    <el-dialog>
                        <dialog id="dialog_{{ $game->id }}" aria-labelledby="dialog-title"
                            class="fixed inset-0 size-auto max-h-none max-w-none overflow-y-auto bg-transparent backdrop:bg-transparent">
                            <el-dialog-backdrop
                                class="fixed inset-0 bg-gray-500/75 transition-opacity data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in"></el-dialog-backdrop>
                            ...
                            <div tabindex="0"
                                ...
                                <el-dialog-panel
                                        ...
                                        <div class="sm:flex sm:items-start">
                                            ...
                                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                                <h3 id="dialog-title" class="text-base font-semibold text-gray-900">
                                                    Usunięcie meczu</h3>
                                                <div class="mt-2">
                                                    <p class="text-sm text-gray-500">Czy na pewno chcesz usunąć ten
                                                        mecz? Wszystkie dane zostaną trwale usunięte. tej akcji nie
                                                        można cofnąć.</p>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                        ...
                                        <form
                                            action="{{ route('profile.deleteGame') }}/{{ $game->id }}/{{ $idCountry }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <x-danger-button
                                        ...
                                        <button type="button" command="close" commandfor="dialog_{{ $game->id }}"
                                            class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150">Anuluj</button>
                                    </div>
                                </el-dialog-panel>
                            </div>
